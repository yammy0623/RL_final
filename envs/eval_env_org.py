...
class EvalDiffusionEnv(gym.Env):
    def __init__(
        ...
        if os.path.isdir(model_name):
            ...
            subfolder = (
                "unet" if os.path.isdir(os.path.join(model_name, "unet")) else None
            )
        ...
        self.scheduler = DDIMScheduler.from_pretrained(
            model_name, subfolder=scheduler_subfolder
        )
        ...
        self.current_image = torch.randn(
            (1, 3, self.sample_size, self.sample_size),
            device="cuda",
    ...
    def step(self, action):
        ...
        with torch.no_grad():
            ### RL step
            interval = (
                self.ddim_scheduler.timesteps[0] - self.ddim_scheduler.timesteps[1]
            )
            ddim_t = self.ddim_scheduler.timesteps[self.current_step_num]
            t = int(
                torch.round(
                    self.ddim_scheduler.timesteps[self.current_step_num]
                    - interval * action
                )
            )
            ...
            if self.current_step_num == 0:
                ...
                input = self.scheduler.add_noise(
                    self.prev_pred_original_image, self.prev_pred_epsilon, t
                ).to("cuda")
            ...
            self.prev_pred_original_image = self.scheduler.step(
                noisy_residual, t, input, generator=self.generator
            ).pred_original_sample
            self.prev_pred_epsilon = self.scheduler.step(
                noisy_residual, t, input, generator=self.generator
            ).pred_epsilon
        ...
        # print('info:', info)
        observation = {"image": self.current_image, "value": t}
